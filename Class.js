...
*/
  ...
  modifySheetRange(modifySheetRangeObj) {
    ...
    if (!ssId) { ssId = this.ssId }
    const rangeObj = [];
    const offsetForCurrentRow = 1;
    const offsetForCurrentCol = 1;
    if (rowDiff > 0) {
      rangeObj.push(this.buildInsertRequest({ sId, dimension: "ROWS", startIndex: currentRange.rowIndex + offsetForCurrentRow, diff: rowDiff }))
    } else if (rowDiff < 0) {
      rangeObj.push(this.buildDeleteRequest({ sId, dimension: "ROWS", startIndex: dataRange.rowIndex, diff: Math.abs(rowDiff) }))
    }
    if (colDiff > 0) {
      rangeObj.push(this.buildInsertRequest({ sId, dimension: "COLUMNS", startIndex: currentRange.colIndex + offsetForCurrentCol, diff: colDiff }))
    } else if (colDiff < 0) {
